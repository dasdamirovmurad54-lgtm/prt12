...
// AniLibria — старый (базовый) API, https://github.com/anilibria/docs/blob/master/api.md
// api.anilibria.tv/v3 не отдаёт CORS-заголовки браузеру, поэтому используем
// легаси-эндпоинт на www.anilibria.tv, который отвечает на form-POST запросы.
const ANILIBRIA_API = 'https://www.anilibria.tv/public/api/index.php';
const PER_PAGE = 24;
...
// ——— AniLibria (легаси API: www.anilibria.tv/public/api/index.php) ———
// Позволяет найти тайтл на AniLibria по названию и посмотреть серии
// прямо в плеере (HLS/m3u8) через hls.js, без ухода с сайта.
// Используем POST с form-urlencoded телом (это "simple request" для CORS,
// без preflight), т.к. api.anilibria.tv/v3 не отдаёт Access-Control-Allow-Origin.
...
async function anilibriaFetch(params) {
  let res;
  try {
    res = await fetch(ANILIBRIA_API, {
      method: 'POST',
  ...
  if (!json || json.status !== true) {
    ...
    throw new Error((err && (err.message || err.description)) || t('anilibriaError'));
  }
  return json.data;
}

async function anilibriaSearch(query) {
  const data = await anilibriaFetch({ query: 'search', search: query, perPage: '8' });
  return (data && data.items) || [];
}

function anilibriaPosterUrl(release) {
  const p = release.poster;
  if (!p) return '';
  return p.indexOf('http') === 0 ? p : 'https://www.anilibria.tv' + p;
}

function anilibriaName(release) {
  const names = release.names || [];
  return names[0] || names[1] || release.code || ('#' + release.id);
}
...
async function anilibriaRunSearch(container, query) {
  ...
  try {
    ...
    if (!results.length) {
      container.innerHTML = '<div class="anilibria-status">' + escapeHtml(t('anilibriaNotFound')) + anilibriaManualLinkHtml(query) + '</div>';
      return;
    }
    anilibriaRenderResults(container, results);
  } catch (err) {
    container.innerHTML = '<div class="anilibria-status">' + escapeHtml(err.message || t('anilibriaError')) + anilibriaManualLinkHtml(query) + '</div>';
  }
}

function anilibriaManualLinkHtml(query) {
  return '<p style="margin-top:10px"><a class="mal-btn" href="https://www.anilibria.tv/search.php?search=' +
    encodeURIComponent(query) + '" target="_blank" rel="noopener">AniLibria.tv →</a></p>';
}
...
function anilibriaRenderResults(container, results) {
  ...
  const cards = results.map(function(r, idx) {
    const names = r.names || [];
    const name = names[0] || names[1] || r.code || ('#' + r.id);
    const sub = names[1] && names[0] ? names[1] : '';
    ...
    return '<div class="anilibria-result-card" data-idx="' + idx + '">' +
      ...
      '<div class="anilibria-result-info">' +
        ...
        (r.type ? '<div class="anilibria-result-sub">' + escapeHtml(r.type) + '</div>' : '') +
      '</div>' +
    '</div>';
  }).join('');
...
function anilibriaRenderPlayer(container, release, allResults) {
  ...
  const name = anilibriaName(release);
  const playlist = release.playlist || [];
  const blockedInfo = release.blockedInfo;
  const blockedNotice = (blockedInfo && blockedInfo.blocked)
    ? '<p class="anilibria-blocked">' + escapeHtml(t('anilibriaBlocked')) + (blockedInfo.reason ? ' — ' + escapeHtml(blockedInfo.reason) : '') + '</p>'
    : '';
  ...
  if (!playlist.length) {
    ...
    episodesHtml = '<div class="anilibria-episodes">' + playlist.map(function(ep, i) {
      const label = ep.title || String(ep.id);
      return '<button type="button" class="anilibria-ep-btn' + (i === 0 ? ' active' : '') + '" data-idx="' + i + '">' + escapeHtml(label) + '</button>';
    }).join('') + '</div>';
  }
  ...
  function playEpisode(idx) {
    ...
    if (!ep) return;
    const src = ep.hd || ep.sd || ep.fullhd;
    if (!src) return;
  ...
  if (playlist.length) playEpisode(0);
}
